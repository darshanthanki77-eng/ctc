...
import { 
  ...
  Clock, Calendar, GitBranch, Database, TrendingUp, ExternalLink, Zap
} from 'lucide-react';
...
const Cron = () => {
  ...
  const handleTriggerRoi = async () => {
    if (!window.confirm('Are you sure you want to manually trigger standard Mining ROI distribution right now? This will process pending active packages.')) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      {/* Live Trading Dedicated Page Banner Callout */}
      <div className="bg-gradient-to-r from-[#161026] via-[#1F143A] to-[#120B20] border border-[#A020F0]/30 rounded-2xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#A020F0]/20 text-[#FF00FF] flex items-center justify-center shrink-0">
            <Zap size={20} />
          </div>
          <div>
            <h4 className="text-sm font-bold text-white">Monthly Live Trading ROI (10% - 15%)</h4>
            <p className="text-xs text-gray-400">Manage, preview, and distribute 1st-of-the-month dynamic Live Trading packages on its dedicated page.</p>
          </div>
        </div>
        <Link
          to="/live-trading"
          className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#A020F0] to-[#FF00FF] hover:from-[#B026FF] text-white text-xs font-bold uppercase tracking-wider rounded-xl shadow-md transition-all shrink-0"
        >
          <span>Open Live Trading Control</span>
          <Zap size={14} />
        </Link>
      </div>

      {/* Main Grid: Left Side (Cron Monitor) & Right Side (Workflow Runs) */}
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-[#0D1117] border border-[#30363D] rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between space-y-5">
            ...
            <button
              ...
              disabled={triggering}
              className="w-full flex items-center justify-center gap-2.5 px-5 py-3.5 bg-[#238636] hover:bg-[#2EA043] disabled:opacity-50 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md shadow-[#238636]/20 cursor-pointer"
            >
              {triggering ? (
                <>
          ...
          <div className="bg-[#0D1117] border border-[#30363D] rounded-2xl p-5 space-y-3">
            ...
            <p className="text-[11px] text-[#8B949E] leading-relaxed">
              Convert USD balances to INR and switch package payouts to INR mode for specific users.
            </p>
            <button
              onClick={handleMigrateInr}
              disabled={migrating}
              className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 bg-[#E3B341] hover:bg-[#C2932E] disabled:opacity-50 text-black text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md font-bold cursor-pointer"
            >
          ...
          <div className="bg-[#0D1117] border border-[#30363D] rounded-2xl p-5 space-y-3">
            ...
            <button
              ...
              disabled={syncing}
              className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 bg-[#238636] hover:bg-[#2EA043] disabled:opacity-50 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md font-bold cursor-pointer"
            >
          ...
          <div className="bg-[#0D1117] border border-[#30363D] rounded-2xl p-5 space-y-3">
            ...
            <p className="text-[11px] text-[#8B949E] leading-relaxed">
              Extend staking period by 20 days from today for completed packages of CTC11893 and CTC33482.
            </p>
            <button
              onClick={handleExtendStaking}
              disabled={extending}
              className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 bg-[#8B2FF7] hover:bg-[#7116DD] disabled:opacity-50 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-md font-bold cursor-pointer"
            >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-4 border-t border-[#21262D] text-xs text-[#8B949E]">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-[#161B22] border border-[#30363D] text-white rounded-lg disabled:opacity-40 hover:bg-[#21262D] transition-all font-semibold cursor-pointer"
                >
                  Previous
                </button>
