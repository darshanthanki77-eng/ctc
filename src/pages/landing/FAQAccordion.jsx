...
const FAQAccordion = () => {
  ...
  const faqs = [
    { q: 'How does CTC achieve 0.002ms execution latency?', a: 'NEXUS uses direct memory access (DMA) kernel acceleration with FPGA hardware offloading, bypassing standard EVM stack delays to process matching logic at raw hardware speeds.' },
    { q: 'Is CTC a non-custodial protocol?', a: 'Yes, 100%. You retain full ownership of your private keys and smart wallet assets at all times. CTC operates entirely via automated zero-knowledge proofs.' },
    ...
    { q: 'What blockchains are supported?', a: 'CTC natively bridges across Ethereum, Arbitrum, Optimism, Solana, Sui, Avalanche, Polygon, Base, and custom enterprise subnets.' },
  ...
  return (
    <section id="faq" className="lp-section-padding lp-section-alt">
      <div className="lp-container" style={{ maxWidth: '820px' }}>
        <div style={{ textAlign: 'center', marginBottom: '52px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.9rem, 3.5vw, 2.9rem)', marginBottom: '14px' }}>
            Clear Answers for <span className="lp-text-gradient">Complex Finance</span>
          </h2>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {faqs.map((faq, idx) => {
            ...
            return (
              <div key={idx} className="lp-glass-card" onClick={() => setOpenIdx(isOpen ? null : idx)} style={{
