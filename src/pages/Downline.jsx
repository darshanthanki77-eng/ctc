...
const TreeCanvas = memo(({ tree, searchQuery }) => {
  ...
  const svgW = Math.max(200, dims.maxX - dims.minX);
  const svgH = Math.max(200, dims.maxY - dims.minY);
  ...
  const centerRoot = useCallback(() => {
    if (!containerRef.current) return;
    const cw = containerRef.current.clientWidth || 900;
    const rootPos = positions[treeData?._id];
  ...
  const toolbarBtns = [
    ...
    { icon: Maximize2,    tip: 'Fit Screen (Auto)', fn: fitScreen },
    { icon: Home,         tip: 'Center Main ID',   fn: centerRoot },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Toolbar */}
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 18px', background:'rgba(255,255,255,0.96)', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(243,16,253,0.12)', flexWrap:'wrap' }}>
        <div style={{ display:'flex', gap:6 }}>
          {toolbarBtns.map(({ icon: Icon, tip, fn }, i) => (
            <button key={i} onClick={fn} title={tip} style={{ width:34, height:34, borderRadius:10, background:'rgba(243,16,253,0.06)', border:'1px solid rgba(243,16,253,0.2)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:'#F310FD', transition:'all 0.18s ease' }}
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:16, alignItems:'center', fontSize:12, fontWeight:700, color:'#64748B' }}>
          <span style={{ display:'flex', alignItems:'center', gap:6 }}><div style={{ width:10, height:10, borderRadius:'50%', background:'#22C55E', boxShadow:'0 0 7px #22C55E' }}/> Active Node</span>
          <span style={{ display:'flex', alignItems:'center', gap:6 }}><div style={{ width:10, height:10, borderRadius:'50%', background:'#94A3B8' }}/> Inactive Node</span>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          ...
          background:'radial-gradient(ellipse at top center, rgba(243,16,253,0.03) 0%, #F8FAFC 100%)',
          minHeight: 620,
          touchAction: 'none'
        }}
        ...
        onWheel={onWheel}
      >
        <div style={{ transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`, transformOrigin:'0 0', position:'absolute', width:svgW, height:svgH, transition: isPanning ? 'none' : 'transform 0.1s ease-out' }}>
          {/* SVG connectors */}
          <svg style={{ position:'absolute', top:0, left:0, width:svgW, height:svgH, pointerEvents:'none', overflow:'visible' }}>
            ...
            {connectors.map(c => (
              <path key={c.id} d={c.d} fill="none" stroke="url(#lineGrad)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity:0.85 }}/>
            ))}
          </svg>
